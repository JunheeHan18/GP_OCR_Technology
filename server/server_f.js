...
app.post('/user/bookmark', function (req, res) {  //북마크
    console.log(req.body);
...
//촬영 명령이 수신되면 작동
 socket.on('take_a_pic', function (msg){ 
     console.log('message:', msg); 
     /*받은 메세지를 되돌려 주자. 
     아니면 받은 데이터를 이용 라즈베리파에서 뭐든 할 수 있다. 
     */ /*
      var pyshell = new PythonShell('python_client2.py');
     pyshell.send(msg);
     ...
     socket.emit('message_from_server', '텍스트변환이 완료되었습니다.'); 
   });
